...
// `cn` (shadcn) absent du projet : concaténation simple suffit.
const cn = (...c: (string | false | null | undefined)[]) => c.filter(Boolean).join(" ");

// `cva` n'était appelé qu'avec une chaîne de base, sans variante : une fonction
// qui la renvoie fait le même travail sans ajouter class-variance-authority.
const cva = (base: string) => () => base;

// `lucide-react` n'est pas installé et n'aurait servi qu'à une icône. Le jeu
// d'icônes maison la fournit déjà.
const ChevronDownIcon = ({ className }: { className?: string }) => (
  <Icon name="chevron-down" size={12} className={className} />
);
